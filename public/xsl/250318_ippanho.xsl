...
<!--
  ...
  【対象情報】
  全般台風情報、全般・地方・府県気象情報（「社会的に影響の大きい天候に関する情報」を除く）、
  天気概況、地方・府県高温注意情報

  【更新履歴】
  2012年03月29日　Ver.1.0
  2019年04月24日　Ver.1.1 5月1日より施行される新元号への対応
  2024年02月16日　Ver.1.1 コメント行の【対象情報】より「全般・地方週間天気予報」
  2025年03月18日　Ver.1.1 コメント行の【対象情報】より「全般スモッグ気象情報、スモッグ気象情報」
を削除
  ======================================================================
-->
...
<xsl:template match="jmx:Report">
	...
	<xsl:if test="h:Head/h:Serial!=''" >
		<xsl:if test="contains(h:Head/h:Title,'　第')=false" >
			<xsl:text>　第</xsl:text>
			<xsl:value-of select="translate(h:Head/h:Serial,'0123456789', '０１２３４５６７８９')" /><xsl:text>号</xsl:text>
		</xsl:if>
	</xsl:if>
	...
	<xsl:call-template name="InsertBlank">
		<xsl:with-param name="value" select="b:Body/b:Notice"/>
	</xsl:call-template>
