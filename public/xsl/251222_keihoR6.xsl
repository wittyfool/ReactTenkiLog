...
<!--
  ...
  【更新履歴】
  2019年04月24日　オリジナル「190501_kei_all_line_div.xsl」Ver.1.2
  2024年10月31日　Ver.1.0（オリジナルから変更。新電文対応版）
  2024年12月26日　Ver.1.0_1 不具合改修版
  2025年12月22日　Ver.1.0_2 不具合改修版（風地域表現修正ほか）
...
<!--ヘッダ部-->
	...
	<!--注意警戒文-->
		<xsl:if test="jmx_ib:Headline/jmx_ib:Text/text()">
		<p class="mtx"><b>
			<xsl:text>（（</xsl:text>
					...
					<xsl:for-each select="jmx_ib:Item">
						...
						<!--解除コード以外の処理-->
							...
							<!--特別警報が発表中（VPWW53orVPNO50で注意警戒文に特別警報の記載あり）の場合-->
								...
								<!--特別警報が発表されていない領域には”なし”を記載 → 誤解を招くとのことなので「－－」に変更-->
										<xsl:otherwise>
											<xsl:text>－－</xsl:text>
										</xsl:otherwise>
									</xsl:choose>
							...
							<!--危険警報--> <!--R06addition -->
							<xsl:if test="count(jmx_ib:Kind/jmx_ib:Name[contains(.,'大雨')!=0 or contains(.,'土砂')!=0 or contains(.,'高潮')!=0])>0">
								<xsl:text>［危険警報］</xsl:text>
								<xsl:choose>
								<!--発表中の警報がある場合-->
									<xsl:when test="count(jmx_ib:Kind/jmx_ib:Name[contains(.,'危')!=0 and contains(.,'特別')=0])>0">
										<xsl:for-each select="jmx_ib:Kind[contains(jmx_ib:Name,'危')!=0 and contains(.,'特別')=0]">
											<xsl:value-of select="substring-before(jmx_ib:Name, '危')" />
										<!--Conditionがある場合は括弧つきで表示 -->
											<xsl:if test="count(jmx_ib:Condition)!= 0">
												<xsl:text>（</xsl:text>
												<xsl:value-of select="jmx_ib:Condition" />
							...
							<!--警報-->
								...
								<xsl:choose>
								<!--発表中の警報がある場合-->
									<xsl:when test="count(jmx_ib:Kind/jmx_ib:Name[contains(.,'警')!=0 and contains(.,'危険')=0 and contains(.,'特別')=0])>0">
										<xsl:for-each select="jmx_ib:Kind[contains(jmx_ib:Name,'警')!=0 and contains(.,'危険')=0 and contains(.,'特別')=0]">
...
<!--ボディ部-->
	<xsl:template match="jmx_mete:Body">
			...
			<!--警報注意報-->
				<xsl:choose>
					<xsl:when test="count(jmx_mete:Kind/jmx_mete:Name) > 0">
						...
						<xsl:call-template name="Status_temp"> <!-- R06addition -->
							<xsl:with-param name="status_param">特別警報から危険警報</xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="Status_temp">
							<xsl:with-param name="status_param">特別警報から注意報</xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="Status_temp"> <!-- R06addition -->
							<xsl:with-param name="status_param">危険警報から警報</xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="Status_temp"> <!-- R06addition -->
							<xsl:with-param name="status_param">危険警報から注意報</xsl:with-param>
						</xsl:call-template>
						<xsl:call-template name="Status_temp">
		...
		<!-- R06addition -->
		<!-- 付加事項部 -->
		<xsl:if test="jmx_mete:AdditionalInfo/jmx_mete:TidalWarningAddition">
			<p class="mtx"><b><font size="+1" color="navy">
				<xsl:text>＜参考情報（高潮予報区間）＞</xsl:text>
			</font></b></p>
			<xsl:for-each select="jmx_mete:AdditionalInfo/jmx_mete:TidalWarningAddition">
				<b><xsl:value-of select="jmx_mete:TidalAreaPart/jmx_mete:Area/jmx_mete:Name" /></b>
				<xsl:text>　（受け持ち区間：</xsl:text>
				<xsl:value-of select="jmx_mete:TidalAreaPart/jmx_mete:ChargeSection" />
				<xsl:text>）</xsl:text><br/>
				<xsl:for-each select="jmx_mete:HydrometricStationPart">
					<xsl:text>　</xsl:text>
					<xsl:value-of select="jmx_mete:Area/jmx_mete:Name" />
					<xsl:text>（</xsl:text>
					<xsl:value-of select="jmx_mete:Area/jmx_mete:Location" />
					<xsl:text>）</xsl:text><br/>
					<xsl:text>　　　</xsl:text>
					<xsl:for-each select="jmx_mete:Criteria/*[@condition='有効']">
...
<!--Statusでまとめる-->
	<xsl:template name="Status_temp">
		...
		<!--危険警報--> <!--R06addition -->
			<xsl:if test="count(jmx_mete:Kind[./jmx_mete:Status=$status_param and contains(./jmx_mete:Name,'危')!=0 and contains(./jmx_mete:Name,'特別警報')=0]) >0 ">
				<xsl:choose>
					<xsl:when test="$status_param!='解除'">
						<b><font color="purple">
							<xsl:choose>
							<!-- 警報の種類が一種類のとき-->
								<xsl:when test="count(jmx_mete:Kind[./jmx_mete:Status=$status_param and contains(./jmx_mete:Name,'危')!=0 and contains(./jmx_mete:Name,'特別警報')=0])=1">
									<xsl:for-each select="jmx_mete:Kind[./jmx_mete:Status=$status_param and contains(./jmx_mete:Name,'危')!=0 and contains(./jmx_mete:Name,'特別警報')=0]">
										<xsl:value-of select="jmx_mete:Name"/>
									<!--Conditionがある場合-->
										<xsl:if test="count(jmx_mete:Condition)!= 0">
											<xsl:text>（</xsl:text>
							...
							<!-- 警報の種類が一種類以上のとき-->
								<xsl:otherwise>
									<xsl:for-each select="jmx_mete:Kind[./jmx_mete:Status=$status_param and contains(./jmx_mete:Name,'危')!=0 and contains(./jmx_mete:Name,'特別警報')=0]">
										<xsl:value-of select="substring-before(jmx_mete:Name, '危')"/>
									<!--Conditionがある場合-->
										<xsl:if test="count(jmx_mete:Condition)!= 0">
											<xsl:text>（</xsl:text>
											<xsl:value-of select="jmx_mete:Condition" />
		...
		<!--警報-->
			<xsl:if test="count(jmx_mete:Kind[./jmx_mete:Status=$status_param and contains(./jmx_mete:Name,'警')!=0 and contains(./jmx_mete:Name,'危険警報')=0 and contains(./jmx_mete:Name,'特別警報')=0]) >0 ">
				<xsl:choose>
					<xsl:when test="$status_param!='解除'">
						<b><font color="red">
							<xsl:choose>
							<!-- 警報の種類が一種類のとき-->
								<xsl:when test="count(jmx_mete:Kind[./jmx_mete:Status=$status_param and contains(./jmx_mete:Name,'警')!=0 and contains(./jmx_mete:Name,'危険警報')=0 and contains(./jmx_mete:Name,'特別警報')=0])=1">
									<xsl:for-each select="jmx_mete:Kind[./jmx_mete:Status=$status_param and contains(./jmx_mete:Name,'警')!=0 and contains(./jmx_mete:Name,'危険警報')=0 and contains(./jmx_mete:Name,'特別警報')=0]">
										<xsl:value-of select="jmx_mete:Name"/>
									<!--Conditionがある場合-->
										<xsl:if test="count(jmx_mete:Condition)!= 0">
											<xsl:text>（</xsl:text>
...
<!--プロパティ-->
	<xsl:template match="jmx_mete:Property" mode="Property_temp">
		...
		<xsl:text>　</xsl:text>
		<xsl:if test="jmx_mete:SignificancyPart"> <!-- R06addition -->
			<!--発表日時-->
			<xsl:if test="../jmx_mete:DateTime[@type='発表時刻']">
				<xsl:text>　　（</xsl:text>
				...
				<xsl:text>日</xsl:text>
				<xsl:value-of select="translate(substring(../jmx_mete:DateTime,12,2),'0123456789', '０１２３４５６７８９')"/>
				<xsl:text>時</xsl:text>
				<xsl:value-of select="translate(substring(../jmx_mete:DateTime,15,2),'0123456789', '０１２３４５６７８９')"/>
				<xsl:text>分更新）</xsl:text>
				<xsl:text>　</xsl:text>
			</xsl:if>
		</xsl:if>
		<br/>
			<!-- R06unnecessary start -->
		<xsl:if test="jmx_mete:WarningPeriod">
			...
			<!-- R06addition start -->
			<xsl:if test="jmx_mete:SignificancyPart">
				<xsl:choose>
					<xsl:when test="jmx_mete:SignificancyPart/jmx_mete:Base/jmx_mete:Local">
						<xsl:for-each select="jmx_mete:SignificancyPart/jmx_mete:Base/jmx_mete:Local">
							<xsl:text>　　　</xsl:text>
							<xsl:value-of select="jmx_mete:AreaName"/>
							<xsl:text>　</xsl:text>
							<xsl:value-of select="jmx_mete:Significancy/@type" />
							<xsl:text>　</xsl:text>
							<xsl:value-of select="jmx_mete:Significancy/jmx_mete:Name" />
							<xsl:if test="jmx_mete:Attention/jmx_mete:Note">
								<xsl:text>（</xsl:text>
								<xsl:value-of select="jmx_mete:Attention/jmx_mete:Note" />
								<xsl:text>）</xsl:text>
							</xsl:if>
							<br/>
						</xsl:for-each>
					...
					<xsl:otherwise>
						<xsl:text>　　</xsl:text>
							<xsl:value-of select="jmx_mete:SignificancyPart/jmx_mete:Base/jmx_mete:Significancy/@type" />
							<xsl:text>　</xsl:text>
							<xsl:value-of select="jmx_mete:SignificancyPart/jmx_mete:Base/jmx_mete:Significancy/jmx_mete:Name" />
						<br/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:if>
			<xsl:if test="jmx_mete:CriteriaPeriod">
				<xsl:text>　　（基準超過の見通し）</xsl:text><br/> <!-- Ver1.0_2 文言変更 -->
				<xsl:choose>
					<xsl:when test="jmx_mete:CriteriaPeriod/jmx_mete:Base/jmx_mete:Local">
						<xsl:for-each select="jmx_mete:CriteriaPeriod/jmx_mete:Base/jmx_mete:Local">
							<xsl:text>　　　</xsl:text>
							<xsl:value-of select="jmx_mete:AreaName"/>
							<xsl:text>　</xsl:text>
					...
					<xsl:otherwise>
						<xsl:text>　　</xsl:text>
							<xsl:value-of select="jmx_mete:CriteriaPeriod/jmx_mete:Base/jmx_mete:Sentence" />
						<br/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:if>
			<xsl:if test="jmx_mete:EventPart">
				<xsl:choose>
					<xsl:when test="jmx_mete:EventPart/jmx_mete:Base/jmx_mete:Local">
						<xsl:for-each select="jmx_mete:EventPart/jmx_mete:Base/jmx_mete:Local">
							<xsl:text>　　　</xsl:text>
							<xsl:value-of select="jmx_mete:AreaName"/>
							<xsl:text>　</xsl:text>
							<xsl:value-of select="jmx_mete:Sentence" />
					...
					<xsl:otherwise>
						<xsl:text>　　</xsl:text>
							<xsl:value-of select="jmx_mete:EventPart/jmx_mete:Base/jmx_mete:Sentence" />
						<br/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:if>
			...
			<xsl:if test="jmx_mete:WindDirectionPart">
				<xsl:choose> <!-- Ver1.0_2 start -->
				<xsl:when test="jmx_mete:WindDirectionPart/jmx_mete:Base/jmx_mete:Local">
						<xsl:for-each select="jmx_mete:WindDirectionPart/jmx_mete:Base/jmx_mete:Local">
							<xsl:text>　　　</xsl:text>
							<xsl:value-of select="jmx_mete:AreaName"/>
							<xsl:text>　</xsl:text>
							<xsl:value-of select="jmx_eb:WindDirection/@description" />
							<br/>
						</xsl:for-each>
				</xsl:when>
				<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="jmx_mete:WaveHeightPart/jmx_mete:Base/jmx_mete:Local">
						<xsl:for-each select="jmx_mete:WaveHeightPart/jmx_mete:Base/jmx_mete:Local">
							...
							<xsl:if test="jmx_mete:Time"> <!-- Ver1.0_2 start -->
								<xsl:text>　（</xsl:text>
								<xsl:call-template name="DateTimeOutput">
									<xsl:with-param name="tmpDateTime" select="jmx_mete:Time"/>
								</xsl:call-template>
								<xsl:text>）</xsl:text>
							</xsl:if> <!-- Ver1.0_2 end -->
							<br/>
					...
					<xsl:otherwise>
						...
						<xsl:if test="jmx_mete:WaveHeightPart/jmx_mete:Base/jmx_mete:Time"> <!-- Ver1.0_2 start -->
							<xsl:text>　（</xsl:text>
							<xsl:call-template name="DateTimeOutput">
								<xsl:with-param name="tmpDateTime" select="jmx_mete:WaveHeightPart/jmx_mete:Base/jmx_mete:Time"/>
							</xsl:call-template>
							<xsl:text>）</xsl:text>
						</xsl:if> <!-- Ver1.0_2 end -->
						<br/>
			...
			<xsl:if test="jmx_mete:TidalLevelPart">
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:if test="jmx_mete:TidalLevelPart/jmx_mete:Base/jmx_mete:Time"> <!-- Ver1.0_2 start -->
							<xsl:text>　（</xsl:text>
							<xsl:call-template name="DateTimeOutput">
								<xsl:with-param name="tmpDateTime" select="jmx_mete:TidalLevelPart/jmx_mete:Base/jmx_mete:Time"/>
							</xsl:call-template>
							<xsl:text>）</xsl:text>
						</xsl:if> <!-- Ver1.0_2 end -->
						<br/>
...
<!-- 年月日分を整形出力するテンプレート --> <!-- Ver1.0_2 テンプレート追加 -->
